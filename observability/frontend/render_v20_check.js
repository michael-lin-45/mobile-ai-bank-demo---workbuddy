...
(async () => {
  ...
  for (let i = 0; i < navCount; i++) {
    ...
    // for intelligence & insight, scroll to capture full content
    const pid = await page.evaluate(() => {
      const a = document.querySelector('.page.active');
      return a ? a.id : 'none';
    });
    await page.screenshot({ path: path.join(outDir, `nav-${i}-${pid}-${label.replace(/[^\w]/g,'_')}.png`), fullPage: true });
    // count canvases with size
  ...
  // 2) AI洞察 tabs: click insight nav then each tab
  await page.evaluate(() => {
  ...
  const tabCount = await page.locator('#p-insight .tab').count();
  console.log('insight tabs:', tabCount);
  for (let i = 0; i < tabCount; i++) {
    await page.locator('#p-insight .tab').nth(i).click();
    await page.waitForTimeout(700);
    const tname = (await page.locator('#p-insight .tab').nth(i).innerText()).trim();
    await page.screenshot({ path: path.join(outDir, `insight-tab-${i}-${tname.replace(/[^\w]/g,'_')}.png`), fullPage: true });
