...
> 配套文档：`docs/specs/AI可观测-系统架构设计-WorkBuddy-V1.md`、`docs/specs/指标GAP分析-v2.md`、`docs/OBSERVABILITY-TARGET-ARCH.md`、`observability/frontend/可观测DEMO-v20-WorkBuddy.html`
...
flowchart TB
    ...
    subgraph OBS2["看板 / 告警 / 前端 (P1/P2)"]
        ...
        FE["React 前端 + 自研大屏 v20<br/>真实数据驱动"]
    end
...
- **后端 → Grafana/前端**：实时查询走 Redis 热层、空则回退 H2；Grafana 统一拼图，前端 v20 大屏直接读 9090 REST。
...
| **前端大屏（v20）** | 8 页（总览/会话/链路/AI洞察/日志/告警/设置/智能洞察）；数据健康三态角标+环形卡；智能洞察独立末位 TAB；总览新增 AI 健康概览四卡；漏斗页流失会话特征画像；Agent KPI 四卡+智能体/工具调用统计图+工具明细表；满意度低分会话明细表+原因分布图；智能洞察页诊断驾驶舱（TOP5 优先行动+三类瓶颈卡+慢会话/不满意根因表+阈值散点） | 接通 9090 真实聚合；三类洞察（性能/准确率/业务）下钻闭环到对应 TAB；诊断驾驶舱可直接指导运营/运维 | 从「设计稿 v19」到「取长补短 v20（V19 框架 + V17-Codex 诊断深度）」 | **P1/P2** |
...
### 6.2 智能洞察建议看板 + 诊断驾驶舱（v20，独立 TAB 放最后）

在 v20 中，智能洞察是一个**独立页面（导航末位）**，页面顶部是「诊断驾驶舱」，下方是 3×3 智能洞察建议网格，视觉区分：紫-蓝渐变头 + 类别彩色左边界。

**A. 诊断驾驶舱（v20 新增，把"看数据"升级为"给行动"）**

页面顶部五个区块，直接指导运营/运维排障：

1. **⚡ 优先行动建议 TOP5**：按 HIGH / MED / LOW 严重度排序的可执行卡（如"理财咨询 P95 2100ms 超阈，扩充并发/缓存"、"转账意图 Reroute 14.2% 偏高，补 L0-L1 分层意图"），每张带严重度标签 + 关联 TAB 下钻。
2. **三类瓶颈卡（性能红 / 准确率黄 / 转化紫）**：左边界按类别着色，汇总该维度 TOP 瓶颈与升级建议，与下方 3×3 网格呼应。
3. **慢会话根因表**：列出 P95 延迟最高的会话（域 / 轮次 / 耗时 / 瓶颈层 / 根因），点击可跳转到对应 trace 或 AI 洞察 TAB。
4. **不满意会话共性表**：汇总低满意度会话的共性（超时占比 / 意图识别错误 / 回答不准确），与满意度 TAB 互证。
5. **Agent P95 延迟 vs 错误率阈值散点**（`chart-diag-perf`）：按 L0/L1/L2 子图分组画柱状 P95 延迟（>1500ms 标红、1000–1500ms 标黄）+ 叠加错误率折线 + 1500ms 阈值虚线，一眼定位"又慢又错"的子图。

**B. 3×3 智能洞察建议网格（三类提炼）**

保留源自 v19 的 3×3 三类提炼项，每卡带**严重度标签 + 下钻链接**（`gotoInsightTab(name)` 自动切到对应 TAB 并激活）：
...
### 6.4 前端大屏 V20 设计增强（取长补短：V19 框架 + V17-Codex 诊断深度）

V20 在 V19 基础上，吸收用户方 V17-Codex 稿"AI 洞察诊断深度"的优点，做 5 处取长补短（Playwright 无头渲染验证：8 页 + 6 个 AI 洞察 TAB 全部渲染，console 零错误）：

| # | 页面 / TAB | V20 增强点 | 借鉴来源 | 价值 |
|---|---|---|---|---|
| 1 | 总览 | 新增 **AI 健康概览四卡**（性能瓶颈 2 / 准确率风险 3 / 转化流失 1 / 满意度 4.1，紫边渐变），点击跳智能洞察/满意度 TAB | V17-Codex | 决策入口一眼看优先级 |
| 2 | AI洞察·漏斗 | 新增 **流失会话特征画像**（按意图 / 轮次 / 原因三分布，提示"60% 流失在 1-2 轮"、"延长 timeout"建议） | V17-Codex | 定位转化流失高发段 |
| 3 | AI洞察·Agent | 升级为 **KPI 四卡（完成率 86.5% / 满意度 4.1 / 转人工 8.2% / 轮次 3.4）+ 智能体调用统计图 + 工具调用统计图 + 4 工具明细表（P95/错误率/典型错误：transfer/queryBill/queryBalance/WealthSearch）** | V17-Codex | 运营可操作的 Agent 视角 |
| 4 | AI洞察·满意度 | 新增 **不满意原因分布图 + 低满意度会话明细表**（Session/User/域/满意度/原因/时长/查看回放） | V17-Codex | 不满意会话可追踪闭环 |
| 5 | 智能洞察（独立页） | 新增 **诊断驾驶舱**（见 §6.2-A：TOP5 优先行动 + 三类瓶颈卡 + 慢会话/不满意根因表 + 阈值散点）；保留 V19 独立 TAB 定位与 3×3 网格 | V17-Codex（嫁接进 V19 框架） | 从"看数据"到"给行动" |

**保留的 V19 基线优势**（未改动）：暗色侧栏 + Manrope / JetBrains Mono 字体、数据健康可信度三态角标（已接通 / 已接待上报 / 待接入）+ 环形卡、日志/告警/设置页已修复 div 嵌套空白 bug、智能洞察独立末位 TAB。

> 设计稿：`observability/frontend/可观测DEMO-v20-WorkBuddy.html`（单文件，ECharts 5.4.3 CDN + 原生 JS 切页）。
> V19↔V17-Codex 优劣判断：V19 胜在骨架正确、架构清晰、字体质感、可信度体系；V17-Codex 胜在 AI 洞察诊断深度与运营可操作性图表；V20 取 V19 框架、嫁接 Codex 诊断深度。
...
> 文档版本：0711 · WorkBuddy V2 ｜ 基于 `AI可观测-系统架构设计-WorkBuddy-V1.md` + `指标GAP分析-v2.md` + 现有代码实测（Core 8080 / Backend 9090 / Collector 4318 / 前端 v20）整理；前端设计已刷新至 v20（取长补短：V19 框架 + V17-Codex 诊断深度）。
