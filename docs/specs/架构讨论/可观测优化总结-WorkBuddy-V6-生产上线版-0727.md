...
# 0728 增补：信创适配生产方案（V6 版）

> **本章节替换 0727 OB 版，基于信创 DB 适配矩阵，适配大多数国内银行。**运行时保留 0718 原文不动，本章节为增量。
...
| 组件 | 范围 | 主推 | JDK |
|---|---|---|---|
| Spring Boot | 3.3~4.0 | **3.5.x** | 17 最低, 21 推荐 |
| Spring AI | 1.0~2.0 | **1.1.x** | 同上 |
| 信创 DB | MySQL系/openGauss系/Oracle系 | 按银行 | — |
...
| **新增组件** | 5~6 个 | **2 个** |
| **审批难度** | ⭐⭐⭐ | ⭐⭐ |
| **适合** | 无国产硬约束, SRE团队, 需跨信号关联 | 国产约束严格, 审批优先不引入新存储 |
| **关系** | B 是底线 → A 是目标, B→A 迁移零业务代码改动 |

## HK-3. 方案 A: 全栈标准版（推荐）⭐

### 推荐条件

- 银行允许引入新开源 DB 组件（VM/Tempo/Loki, 均为 CNCF, 不涉及费用）
- 可观测系统独立部署, 不与核心交易耦合
- 需要跨信号关联排查（trace→log→metric 下钻）
- 日均 Span > 2000 万（专用存储比通用 DB 更合适）

### 组件选型

| 信号 | 组件 | 许可 |
|---|---|---|
| 采集 | OTel Agent + Collector | Apache 2.0 |
| Trace | **Grafana Tempo** | Apache 2.0 |
| Metrics | **VictoriaMetrics** 或 Mimir | Apache 2.0 |
| Log | **Grafana Loki** | AGPLv3 |
| 看板 | **Grafana** | AGPLv3 |
| 业务真相源 | 信创 DB（sessions/业务指标） | — |
| LLM 工程 | 按信创 DB 兼容性选 Langfuse 或 MLflow | — |

### 审批说明

> 新增 5~6 个开源组件（均为 CNCF 生态标准，Apache 2.0/AGPLv3 许可，不涉及商业费用），已��数亿用户验证（Uber/Netflix/LinkedIn）。可观测系统独立部署。Tempo+Loki+Grafana 三位一体跨信号关联排查，是业界公认标准栈。
...
- 国产化约束严格, 审批优先"不引入新存储"
- 信创 DB 性能可满足初期量级（日均 Span < 2000 万）
- 自研前端 v24 已覆盖展示需求
...
| 信号 | 组件 | 说明 |
|---|---|---|
| 采集 | **OTel Agent** + **OTel Collector** | **仅此 2 个新组件** |
| Trace | 信创 DB spans 表 + 自研查询 | 复用已有 |
| Metrics | 信创 DB metrics_agg 表 | 量级小时够用 |
| Log | 信创 DB logs 表 + 大数据系统 | 复用已有 |
| Session | 信创 DB sessions 表 | 复用已有 |
| 展示 | 自研前端 v24 + 可选 Grafana | 已有 |

### 审批说明

> 仅新增 2 个 CNCF 毕业组件。存储层全部复用银行已有信创 DB/Redis/Kafka/大数据系统/BI，零新存储。trace/log/metrics/session 全用信创 DB 存。

## HK-5. 递进关系 + 决策树

```
方案 B（底线）→ 任何银行都能落地，先跑通验证价值
        ↓ 条件成熟
方案 A（目标）→ 加 Tempo+VM+Loki+Grafana，升级专业栈
        ↓
迁移成本 = 零！OTLP 格式相同，只多配 exporter
```
...
> **V6 增补版本**：0728 v3 · 双方案结构（方案A全栈标准 + 方案B国产化务实）
> **冲突优先级**：本增补 > 0718 > 0727 OB
> **前置阅读**：`可观测架构业界调研与重构建议-GLM5.2-0726.md` + `银行可观测落地 FAQ.md`
