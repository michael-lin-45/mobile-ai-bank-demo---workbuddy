...
# 0728 增补：通用 Demo 演示方案（V4-Demo 版）

> **本章节替换 0727 OB 版。V4 Demo 定位为本机通用 AI 可观测演示系统，不绑特定银行，可在实验室或本机 10 分钟跑通。**
> 调研依据：`docs/specs/架构讨论/可观测架构业界调研与重构建议-GLM5.2-0726.md` §十三

## DM-1. Demo 定位

- **不是**某家银行的系统复刻
- **是**一套可在本机（Windows/macOS/Linux）快速启动的 AI 可观测演示系统
- 展示完整链路：埋码 → 采集 → 存储 → 展示
- 给银行客户做 POC 演示，体现技术能力

## DM-2. 三套方案对比

| | 方案 A：极简 | 方案 B：推荐 ⭐ | 方案 C：完整 |
|---|---|---|---|
| **DB** | H2 文件库（嵌入式） | H2 文件库 + Langfuse 容器内 PG+ClickHouse | PostgreSQL 外部安装 |
| **Langfuse** | ❌ | ✅ docker compose | ✅ 生产模式 |
| **OTel Agent** | ✅ | ✅ | ✅ |
| **OTel Collector** | ✅ | ✅ | ✅ |
| **自研 Backend** | H2 版 | H2 + OTLP → LF | PG 版 |
| **自研前端** | v24 7页 | v24 7页 | v24 7页 |
| **启动耗时** | <5 分钟 | <10 分钟 | <20 分钟 |
| **外部依赖** | 无 | Docker | Docker + PG |
| **适合场景** | 快速 POC | **功能最全演示** | 生产演练 |
...
├── OTel Collector (4318)      — 采集网关，batch+filter
├── Backend (9090)             — H2 文件库 + OTLP Receiver + API
├── 前端 (3000)                — Vite + React v24 7页
└── Docker Compose:
...
### 步骤 3：Core 启动（含 OTel Agent）

```bash
java -javaagent:opentelemetry-javaagent.jar \
     -Dotel.service.name=bank-ai-demo \
     -Dotel.exporter.otlp.endpoint=http://127.0.0.1:4318 \
     -jar bank-ai-core.jar --server.port=8080
```

### 步骤 4：OTel Collector 启动
...
exporters:
  otlphttp/backend:
    ...
    compression: gzip
  otlphttp/langfuse:
    endpoint: http://127.0.0.1:3001/api/public/otel/v1/traces
service:
  pipelines:
    traces:
      receivers: [otlp]
      processors: [memory_limiter, batch]
      exporters: [otlphttp/backend, otlphttp/langfuse]
    metrics:
      receivers: [otlp]
      processors: [memory_limiter, batch]
...
### 步骤 5：Langfuse docker compose

```bash
curl -O https://raw.githubusercontent.com/langfuse/langfuse/main/docker-compose.yml
# 编辑 docker-compose.yml，修改密码标记为 # CHANGEME 的字段
docker compose up -d
# 访问 http://localhost:3000 注册账号
```

### 步骤 6：自研 Backend + 前端启动

```bash
# Backend (H2 文件库，零配置)
cd observability/backend
nohup java -jar target/backend.jar --server.port=9090 &
...
| 验证项 | 检查方法 |
|---|---|
| Core OTLP 发送 | `curl http://127.0.0.1:4318/v1/traces` 无 404 |
| Backend 接收 | `GET http://127.0.0.1:9090/api/v1/traces/debug/span-stats` 有数据 |
| Langfuse 接收 | 浏览器打开 http://localhost:3000 看 trace 列表 |
| 前端展示 | 浏览器打开 http://localhost:3000 看 7 页数据 |

## DM-5. 方案 A（极简）与方案 B 的差异

方案 A 去掉步�� 5（Langfuse），Collector 配置去掉 `otlphttp/langfuse` exporter。其他步骤完全相同。

## DM-6. Demo 展示能力清单

| 页面 | 展示内容 | 数据来源 |
|---|---|---|
| 总览大屏 | DAU/在线/QPS/Token/TTFT/错误率 | OceanBase/H2 + Redis |
| 会话回放 | 用户输入+AI回复+内部Graph详情 | sessions + session_turns 表 |
| 链路追踪 | trace 瀑布图+span 详情含 intent/confidence | spans 表 |
| AI 洞察(7 TAB) | 智能诊断/准确率/Agent性能/转化漏斗/Token成本/外部调用/满意度 | spans + metrics_agg 聚合 |
| 日志查询 | 按 traceId/sessionId 关联查询 | logs 表 |
| 告警规则 | 规则配置+事件触发 | alert_rules + alert_events |
| 系统设置 | 提示词管理(方案B用Langfuse UI, 方案A自研) | prompt_versions 或 Langfuse |

## DM-7. Java 插桩三种场景（适配不同银行业务系统）

> 不同银行的业务系统可能用不同框架：有的用 Spring AI，有的只用 Spring Boot 裸调 LLM，有的甚至是纯 Java 服务。Demo 方案需要覆盖三种场景。

### 场景 A：纯 OpenTelemetry（无 Spring AI 框架）

**适用**：银行已有裸 Java 服务，LLM 调用走 HTTP API（如 OkHttp 调 DashScope/OpenAI）

| 层 | 实现 | 代码量 |
|---|---|---|
| **L1 基础设施** | OTel Java Agent 自动采 HTTP/JDBC/Kafka | 零代码 |
| **L2 AI 应用** | OTel SDK 手动创建 LLM 调用 span | 中（需手写 Tracer.spanBuilder） |
| **L3 业务语义** | OTel SDK 手动 Counter/Timer | 中 |

```java
// 纯 OTel SDK 手动埋 LLM 调用
Tracer tracer = GlobalOpenTelemetry.getTracer("bank-ai");
Span span = tracer.spanBuilder("llm.call")
    .setAttribute("gen_ai.system", "dashscope")
    .setAttribute("gen_ai.request.model", "qwen-plus")
    .startSpan();
try (Scope scope = span.makeCurrent()) {
    // 调 LLM API
    long start = System.nanoTime();
    Response resp = httpClient.call(llmApi, requestBody);
    long duration = System.nanoTime() - start;
    span.setAttribute("gen_ai.response.duration_ms", duration / 1_000_000);
    // Token 需从响应体解析
    span.setAttribute("gen_ai.usage.input_tokens", parseInputTokens(resp));
    span.setAttribute("gen_ai.usage.output_tokens", parseOutputTokens(resp));
} finally {
    span.end();
}
```

**特点**：无 gen_ai.* 自动指标，Token/TTFT/错误率全手写。工作量最大。

### 场景 B：Spring AI + OpenTelemetry（推荐）⭐

**适用**：银行用 Spring AI 框架（1.0+），ChatClient/ChatModel 调用 LLM

| 层 | 实现 | 代码量 |
|---|---|---|
| **L1 基础设施** | OTel Java Agent 自动采 HTTP/JDBC/Kafka/Reactor | 零代码 |
| **L2 AI 应用** | Spring AI Observation 自动采 gen_ai.* + ObsChatModel 补 TTFT | 少量 |
| **L3 业务语义** | Micrometer 手动 Counter/Timer（Reroute率/完成率/转人工） | 少 |

> **`+ ObsChatModel` 不是第 4 层，是 L2 层的手工补充**——Spring AI 1.1.x 自动有 token usage 但无 TTFT，需装饰器补。标为"补充"而非独立层，避免误解为 4 种埋码方式。

```yaml
# application.yml — 一行配置开启 AI 层观测
spring:
  ai:
    chat:
      observations:
...
Spring AI 自动产 `gen_ai.client.operation.duration` + `gen_ai.client.token.usage`。只需手写 ObsChatModel 补 TTFT + 业务指标。

**版本差异**：
| Spring AI 版本 | 自动指标 | 需手写补 |
|---|---|---|
| 1.0.x | operation.duration | Token + TTFT + 错误分类 |
| 1.1.x | operation.duration + token.usage | TTFT + 业务指标 |
| 2.0.x | 全链路 span + token + tool | 业务指标（TTFT 待确认） |

### 场景 C：Spring Boot + OpenTelemetry（无 Spring AI）

**适用**：银行用 Spring Boot 但没用 Spring AI，LLM 调用走 OkHttp/RestTemplate/WebClient

| 层 | 实现 | 代码量 |
|---|---|---|
| **L1 基础设施** | OTel Java Agent 自动采 | 零代码 |
| **L2 AI 应用** | OTel SDK 手动 span（同场景 A） | 中 |
| **L3 业务语义** | Micrometer 手动（Boot Actuator 已有） | 少 |

特点：有 Spring Boot 的 Micrometer 基础设施（Actuator + MeterRegistry），但没有 Spring AI 的自动 AI 观测。LLM 调用仍需手写 span。

### 三场景对比总结

| 维度 | 场景 A（纯 OTel） | 场景 B（SAI+OTel）⭐ | 场景 C（Boot+OTel） |
|---|---|---|---|
| gen_ai.* 指标 | ❌ 全手写 | ✅ 自动 | ❌ 全手写 |
| Token 采集 | ❌ 手写 | ✅ 自动（1.1+） | ❌ 手写 |
| TTFT | ❌ 手写 | ⚠ ObsChatModel 补 | ❌ 手写 |
| 业务指标 | ❌ 手写 | ❌ 手写 | ❌ 手写 |
| HTTP/JDBC span | ✅ Agent 自动 | ✅ Agent 自动 | ✅ Agent 自动 |
| **推荐度** | ⭐⭐ | ⭐⭐⭐ | ⭐⭐ |

**Demo 建议**：Demo 系统用场景 B（Spring AI + OTel）实现，但在文档中说明三种场景的适配方法，方便不同银行按实际情况选型。

## DM-8. Langfuse 依赖澄清

### Langfuse v3 组件依赖（必需，不可省略）

| 组件 | 是否必需 | 用途 | 能否去掉 |
|---|---|---|---|
| **PostgreSQL** | ✅ 必需 | Langfuse 状态存储（用户/项目/配置） | ❌ 不能 |
| **ClickHouse** | ✅ 必需 | OLAP 存储（Trace/Observation/Score） | ❌ 不能 |
| **Redis** | ✅ 必需 | 异步摄取队列 | ❌ 不能 |
| **S3/MinIO** | ✅ 必需 | Blob 存储（大 payload/多媒体） | ❌ 不能 |

### Langfuse v2（已 EOL，不推荐）

Langfuse v2 仅需 PostgreSQL，但已于 2025 Q1 结束支持，不推荐新项目使用。

### Demo 方案 B 的 Langfuse 部署建议

```bash
# 一条命令拉起 Langfuse v3 + 全部依赖（PG+ClickHouse+Redis+MinIO 均在容器内）
curl -O https://raw.githubusercontent.com/langfuse/langfuse/main/docker-compose.yml
# 修改 # CHANGEME 标记的密码字段
docker compose up -d
```
...
**如果不想装这么多容器**：
- → 选方案 A（不引入 Langfuse，自研提示词管理表）
- → 或选 Langfuse Cloud 免费版（50k observations/month，不需自部署）

### 生产环境（银行）结论

按 FAQ §五结论：**Langfuse 在银行基本不可行**（强依赖 PG，银行无 PG）。替代方案：
- 提示词管理 → 自研 OceanBase 表
- LLM 评估 → DeepEval（开源，Python，不绑 PG）
- 实验追踪 → MLflow（MySQL 兼容，OceanBase 可直接用）

---

## DM-9. 关键澄清：ObsChatModel 与 Micrometer 的关系

### ObsChatModel 不是独立框架，是 Micrometer 的场景封装
...
```
    ...
    ┌──────────────────┐              ┌──────────────────┐
    │ ObsChatModel     │              │ 业务代码          │
    │ 用 Micrometer 补  │              │ 用 Micrometer 写  │
    │ TTFT + LLM指标   │              │ Reroute/完成率等  │
    │ llm.* 前缀       │              │ deepflux.* 前缀   │
    └──────────────────┘              └──────────────────┘
           共享同一个 MeterRegistry，都走 OTLP
```

**结论**：
- ObsChatModel **用** Micrometer API 写指标，不是替代 Micrometer
- 业务代码也**用同一个** Micrometer MeterRegistry，通过命名前缀区分（`llm.*` vs `deepflux.*`）
- 技术上可以把业务指标也写在 ObsChatModel 里，但**不推荐**——ObsChatModel 的职责是 ChatModel 装饰器（LLM 调用范畴），关注点分离原则建议各管各的

### VM/Tempo/Loki 去哪了？

**没有被砍掉，是被更务实的方案替代了。**

| 阶段 | trace 存储 | metrics 存储 | log 存储 |
|---|---|---|---|
| **调研推荐（v1.0）** | Tempo | VictoriaMetrics | Loki |
| **有 ES 的银行（v1.2）** | ES（替代 Tempo） | MySQL（替代 VM） | ES（替代 Loki） |
| **无 ES 的银行（v1.4）** | OceanBase spans 表 | OceanBase metrics_agg 表 | OceanBase logs 表 |
| **Demo（v1.5）** | H2 spans 表 | H2 metrics_agg 表 | H2 logs 表 |

**Demo 为什么不装 VM/Tempo/Loki**：
- Demo 目标是**10 分钟本机启动**，H2 嵌入式数据库零配置
- 自研 Backend + 自研前端已经覆盖了 trace/metric/log 的查询展示
- VM/Tempo/Loki 的价值在于生产级性能和标准协议兼容（PromQL/TempoQL/LogQL），Demo 阶段不需要

**如果要展示 VM/Tempo/Loki**：在方案 C（完整方案）里，可以用 docker compose 起 VM+Tempo+Loki 三件套 + Grafana 统一看板，作为"生产级演示"的增强选项。

### Demo 目标架构（方案 B 推荐版）

```
┌────────────────────────────────────────────┐
│  Core (8080) — Spring AI + OTel Agent     │
│  L1 Agent + L2 SAI Obs + L3 业务手工       │
└──────────────────┬─────────────────────────┘
                   │ OTLP :4318
┌──────────────────▼─────────────────────────┐
│  OTel Collector (单实例)                    │
│  batch + filter → otlphttp/backend         │
│           → otlphttp/langfuse (可选)        │
└──────────────────┬─────────────────────────┘
                   │ OTLP
        ┌──────────┴──────────┐
        ▼                     ▼
┌───────────────┐  ┌──────────────────────┐
│ Backend (9090)│  │ Langfuse (可选,3001)  │
│ H2 文件库     │  │ docker compose 启动   │
│ 自研查询 API  │  │ PG+CH+Redis+MinIO     │
└───────┬───────┘  └──────────────────────┘
        │
┌───────▼───────────────────────────────────┐
│  自研前端 (3000) — v24 7页                 │
...
| 层 | 组件 | 是否新增 |
|---|---|---|
| 埋码 | Spring AI Observation（已有）+ OTel Java Agent（新增） | 1 个新增 |
| 采集 | OTel Collector（新增） | 1 个新增 |
| 存储 | H2 文件库（已有）+ Langfuse 容器内 PG/CH/Redis/MinIO（可选） | 0 个新增（容器内） |
| 查询 | 自研 Backend（已有） | 0 |
| 展示 | 自研前端 v24（已有）+ Langfuse UI（可选） | 0 |

**仅新增 2 个独立部署组件**：OTel Java Agent + OTel Collector，其余全部复用已有或容器内封闭。H2 零配置，10 分钟可启动。

---

> **V4-Demo 增补版本**：0728 v3 · + ObsChatModel/Micrometer 澄清 + VM/Tempo/Loki 演进说明 + Demo 目标架构
> **冲突优先级**：本增补章节 > 0718 原文 > 0727 OB 版
> **前置阅读**：`可观测架构业界调研与重构建议-GLM5.2-0726.md` §十三 + `银行可观测落地 FAQ.md`
