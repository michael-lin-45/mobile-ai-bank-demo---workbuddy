...
> 版本：V1 | 日期：2026-07-11 | 作者：Codex（AI 搭档）
> 基于《AI可观测-系统架构设计-WorkBuddy-V1.md》+《指标GAP分析-v2.md》+ 可观测DEMO-v17-Codex.html + 现有代码深度分析
...
| 6 | **洞察停留在"看数据"** | 6 个洞察 TAB 各自独立展示指标，无交叉分析、无根因定位、无行动建议 | 运营/运维/算法需要人工交叉分析 |
...
| **洞察** | 智能诊断 TAB | ❌ 不存在 | 新增第 7 TAB：交叉分析+根因+行动建议 | P0 | 后端: InsightsEngineService + 前端 |
...
| **自研前端** | 6 TAB + 3s 轮询 | 7 TAB（+智能诊断）+ 轮询优化 | 新增诊断 TAB + 微调 |
...
graph TB
    ...
    subgraph 前端["可观测前端 (React :3000)"]
        RT[React Router<br/>7页面]
        subgraph 页面["7页面"]
            DP[总览大屏<br/>+AI健康概览]
            SVP[会话回放]
            TEP[链路追踪<br/>+置信度/分层意图列]
            AIP[AI洞察<br/>7 TAB]
            LVP[日志查询]
            ARP[告警规则]
            SSP[系统设置]
...
| D5 | 智能诊断作为第 7 TAB | 不替换现有 6 TAB，在其上增加交叉分析层 |
...
- **终稿文件**：`observability/frontend/可观测DEMO-v17-Codex.html`
- **设计风格**：深色主题（Dark Mode），与 Grafana/Langfuse 风格对齐
- **技术栈**：纯 HTML + Tailwind CSS（CDN） + Chart.js + ECharts
- **页面数量**：7 个主页面 + AI 洞察 7 TAB 子页面

### 11.2 页面导航结构

```
顶部导航栏
├── 总览大屏 (Overview)        — AI 健康概览 + 核心指标实时面板
├── AI 洞察 (Insights)         — 7 TAB 智能诊断中心
│   ├── 性能瓶颈
│   ├── 慢会话根因
│   ├── 不满意模式
│   ├── 准确率分析
...
│   ├── 行动建议
│   └── 趋势预测
├── 链路追踪 (Traces)          — Span 表格 + 调用树可视化
├── 会话分析 (Sessions)        — 会话列表 + 单会话详情
├── 指标监控 (Metrics)         — 多维度指标面板
├── 日志检索 (Logs)            — 全文检索 + 结构化过滤
├── 告警中心 (Alerts)          — 活跃告警 + 历史告警 + 规则管理
└── 系统设置 (Settings)        — 数据源配置 + 告警渠道配置
```

### 11.3 总览大屏 - AI 健康概览卡片

总览大屏顶部新增 **AI 健康概览卡片**，聚合展示 AI 应用整体健康度：

| 指标 | 展示形式 | 数据来源 |
|------|---------|---------|
| AI 准确率 | 环形进度条 + 趋势箭头 | `obs:health:ai -> accuracy` |
| 用户满意率 | 环形进度条 + 趋势箭头 | `obs:health:ai -> satisfaction` |
| 平均响应延迟 | 数字 + P95/P99 标签 | `obs:health:ai -> avg_latency` |
| 错误率 | 数字 + 红色告警阈值标记 | `obs:health:ai -> error_rate` |
| 日均 Token 消耗 | 数字 + 配额进度条 | `obs:quota:daily -> token_total` |
| 活跃会话数 | 数字 + 在线人数 | `obs:sessions:active` |
| AI 健康综合分 | 仪表盘 (0-100) | 以上指标加权计算 |

### 11.4 AI 洞察 7 TAB 结构详解

**TAB 1: 性能瓶颈**
- Top3 慢 Agent 节点排行榜（柱状图 + 趋势线）
- LLM 响应延迟分布直方图
- Token 消耗异常检测时序图（标注异常点）

**TAB 2: 慢会话根因**
- 慢会话列表（可点击展开根因链路图）
- 根因链路树形可视化（独占时间高亮）
- 根因维度关联面板（LLM指标 / 工具调用 / 意图 / 上下文）
...
**TAB 6: 行动建议**
- 优先级行动建议 Top10 列表
- 每条建议含：影响面 / 严重度 / 可修复性 / 预计收益
- 一键标记"已采纳" / "已忽略" / "处理中"

**TAB 7: 趋势预测**
- 关键指标 7天预测线（基于历史趋势）
- 容量预警（Token 用量 / QPS 预测）
- 建议扩容时间点标注

### 11.5 链路追踪表格新增列

相比 V16 版本，V17 链路追踪表格新增以下列：

| 新增列 | 说明 | 数据来源 |
|--------|------|---------|
| 置信度 (Confidence) | LLM 输出置信度评分 (0-1) | `llm_metrics.confidence_score` |
| 分层意图 (Layered Intent) | 三层意图分类：L1领域/L2场景/L3动作 | `spans.attributes -> intent.l1/l2/l3` |
| Token 消耗 | 输入/输出 Token 数 | `llm_metrics.input_tokens / output_tokens` |
| 用户反馈 | 满意度评分 (1-5) | `sessions.feedback_score` |
| 流失标记 | 是否为流失会话 | `sessions.is_converted = false` |

### 11.6 漏斗流失特征画像

转化瓶颈 TAB 中的流失会话特征画像，从以下维度构建：
...
- **配色方案**：背景 #0F172A / 卡片 #1E293B / 主色 #3B82F6 / 告警 #EF4444 / 成功 #10B981
- **响应式布局**：支持 1920px / 1440px / 平板自适应
- **图表库**：Chart.js（折线/柱状/饼图）+ ECharts（桑基图/热力图/雷达图）
- **字体**：Inter（英文）+ 思源黑体（中文），均使用 CDN 引入
- **无后端依赖**：V17 为纯前端原型，所有数据使用 Mock JSON，可直接用浏览器打开预览
...
**目标**：前端 V17 对接后端 API，全链路联调

| 任务 | 涉及组件 | 工作量 | 优先级 |
|------|---------|--------|--------|
| V17 前端 API 对接 | 替换 Mock 数据为真实 API | 2天 | P2 |
...
| # | 优化描述 | 收益 | 实现方式 |
|---|---------|------|---------|
| P2-1 | 前端 V17 API 全对接 | 实时数据展示 | 替换 Mock 数据为真实 API 调用 |
