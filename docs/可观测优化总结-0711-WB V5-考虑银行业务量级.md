# 可观测优化总结（0711 · WorkBuddy V5 · 银行业务量级版）

> **V5 背景**：银行业务系统，静态用户量数千万级。V4 的 Demo 假设（H2 扛全量、Redis 单实例、Collector 单机）在银行量级下需全面升级。
> **V5 变更要点**：H2→PostgreSQL 从 P2 提前到 P0；Redis 单实例升级为 Sentinel 高可用 + DAU 从 HyperLogLog 改为 Bitmap（精度 100%）；Collector 集群化 + LB；采样策略从 10% 降为 1%；新增分层数据保留策略（热 7d→温 90d→冷 3y 银保监合规）；告警增加收敛窗口与静默期。
...
| **P1** <br/>Collector 增强<br/>+ 标准栈<br/>+ AI 洞察引擎 | Collector 处理器增强；<br/>标准栈信号外溢；<br/>AI 洞察引擎生产级 | ① Collector 加 batch / filter / tail_sampling（降为 1% 正常采样） / attributes（§12）<br/>② **部署 Tempo + VictoriaMetrics(集群) + Loki + Grafana(HA)**（docker-compose）<br/>③ Collector 加 Tempo/VM/Loki exporter<br/>④ InsightsEngineService（交叉分析+根因+建议, §10）<br/>⑤ **分层数据保留策略**（热 7d PG → 温 90d 压缩分区 → 冷 3y 对象存储，银保监合规）<br/>⑥ OpenLLMetry 可选引入（gen_ai.* 语义） | 1.5–2 人周 | 🟢 后端 6 TAB 实时聚合已重写 |
| **P2** <br/>告警迁移<br/>+ 前端终稿 | 告警迁 Grafana；<br/>前端真实数据驱动 | ① 告警从自研迁 Grafana Alerting<br/>② 前端 V20 全 API 对接（替换 Mock）<br/>③ 端到端测试 + 压测验证<br/>④ PG 读写分离（读副本） | 1–1.5 人周 | ⚪ 待启动 |
| **P3** <br/>深化 + LLM 专项 | 第四信号、LLM 专项、安全审计 | Langfuse（待定项 C）/ Pyroscope / Sentry(Faro) / Beyla / TLS 全链路加密 / 审计日志 / 模型对比评估 | 持续 | ⚪ 待决策 |
  ...
  tail_sampling:
    ...
    policies:
      ...
      - name: normal-sampling
        type: probabilistic
        probabilistic: { sampling_percentage: 1 }   # ★ V5: 银行量级降为 1%（亿级×1%=百万级/天，可管理）
...
> 文档版本：0711 · WorkBuddy V5（银行业务量级版 · 基于 V4 生成）
> 融合路径：WorkBuddy V2 + Codex V2 → Wb V3 + Codex V3 → Wb V4 → Wb V5（银行量级修正）
> V5 修正依据：银行业务系统，静态用户量数千万级 → H2→PG 提前 P0、Collector 集群化、Redis Sentinel、Bitmap DAU、分层保留 3y、告警收敛+静默
