# 可观测优化总结（0711 · WorkBuddy V6 · web文章合入 · 银行生产终稿）
...
> **web文章合入要点**（15 篇外部参考文章，22 条可采纳观点 P1–P22，全部注入）：Kafka 缓冲层 / batch 纠偏 / Exemplar / HITL 会话连续性 / LLM SLO 框架 / 集中指标注册表 / GenAI 版本钉注 / DaemonSet / AI 自身可观测+PII 网关级 BLOCK / SLO Tier 分级 / 时序折叠蓝图 / 三层边界 / MCP+RAG+CAG / 动态基线 / 7 层黄金信号 / SLO 影响分析降噪 / Owner 必填 / 提示词即 Runbook / IDP 推广 / 采样细化。
...
> 外部文章依据：`docs/外部参考文章与自有方案对比分析.md`（15 篇微信文章，P1–P22 分级清单，§8.5 Demo/生产分界表）
> 组件选型最优组合：**Alloy(采集集群) + Tempo(Trace) + VictoriaMetrics(集群) + Loki(日志) + Kafka(缓冲) + Grafana(HA)** —— 全 Grafana 系 + Kafka 缓冲，生产级高可用。

---

## 前置说明：web文章合入范围（15 篇外部文章 → V6 生产级全量注入）

> **原则**：15 篇外部文章 90% 观点对准生产环境，与 V6 的"银行量级生产"定位天然吻合。本文合入全部 22 条可采纳观点（P1–P22），对 V6 原有设计做精准补强，**不改组件选型、不改阶段划分**。

| 合入观点 | 来源 | 落地位置 | 类型 |
|---|---|---|---|
| P1 Kafka 缓冲层 | A3 | §2 架构 + §11 Collector + §14 HA | 🔴 组件新增 |
| P2 batch processor 纠偏 | A2 | §11 Collector 配置 | 🔴 风险纠偏 |
| P3 Exemplar 显式化 | A3 | §4.3 指标关联 | 🔴 机制补强 |
| P4 HITL 跨 trace 会话连续性 | A1 | §4.2 Trace 设计 | 🟡 设计补强 |
| P5 LLM 推理 SLO 框架 | A2 | §4.3 Metrics + 新 § SLO | 🟡 指标补强 |
| P6 集中式指标注册表 | A1 | §5 Core 埋点 | 🟡 代码规范 |
| P7 UpDownCounter 待审批数 | A1 | §5 + §6 指标全表 | 🟡 新增指标 |
| P8 GenAI 语义版本钉注 | A2 | §5.3 强制约定 | 🟡 脚注提醒 |
| P9 Collector DaemonSet | A3 | §11 + §14 HA | 🟡 部署补强 |
| P10 AI 自身可观测 + PII 护栏 | N14 | §4.3 + §13 合规 | 🔴 银行必须 |
| P11 SLO 从业务向下拆 + Tier 分级 | N10/N11 | 新 §「SLO 分级设计」 | 🔴 设计重写 |
| P12 金融落地蓝图「时序折叠」 | N11 | §14 实施路径 | 🔴 路线图 |
| P13 三层边界治理 InsightsEngine | N9 | §9 AI 洞察引擎 | 🟡 架构补强 |
| P14 MCP 集成 | N9 | §9 InsightsEngine | 🟡 扩展架构 |
| P15 RAG/CAG 分工 | N9 | §9 InsightsEngine | 🟡 知识来源 |
| P16 动态基线替代静态阈值 | N10/N11 | §10 告警引擎 | 🟡 告警策略 |
| P17 云原生 7 层黄金信号 | N10 | §4 指标分层 | 🟡 分类框架 |
| P18 SLO 影响分析降噪 / burn rate | N10/N6 | §10 告警引擎 | 🟡 降噪机制 |
| P19 可观测性驱动开发 + Owner 必填 | N10/N12 | 新 §「平台治理」 | 🟡 流程规范 |
| P20 提示词即 Runbook | N9 | §9 InsightsEngine | 🟡 版本管理 |
| P21 IDP/Helm/Kyverno/成熟度 | N12/N13/N15 | 新 §「多团队推广」 | 🟡 可选章节 |
| P22 银行采样细化 | N10 | §11 Collector 分级采样 | 🟡 策略细化 |
...
- 日志全量带 `session_id` + `trace_id`
- 查询：从「同一 session_id 跨越两条 trace」的日志行中还原完整业务线
- 前端"会话回放"页以 session_id 为单位，但"链路详情"页标注"HITL 中断 — 跳转关联 trace"

```
用户发起转账 → Trace-A (HTTP/check)
      ↓ 人工审批（中断点：无 trace 覆盖）
审批通过 → Trace-B (HTTP/confirm)
      ↓
Loki: session_id=s_123, trace_id=A → "等待审批"
Loki: session_id=s_123, trace_id=B → "审批已通过"
→ 会话回放页缝合两条 trace 为一条业务时间线
```
...
**★web合入 P3：Exemplar 机制**（来自 A3《Java微服务+K8s: OTel三信号统一》）

指标数据点须携带 `trace_id` 作为 Exemplar，实现 **"告警 → Grafana → Exemplar → Trace → Log"** 一键下钻链：
- VM/Prometheus 原生支持 Exemplar（RED 指标上开启）
- Core 侧：`Timer` 记录时自动关联当前`trace_id`（OTel javaagent 已支持）
- Grafana 面板：告警面板数据点可点击 → 跳转 Tempo trace → 再跳 Lokilog
- 适用指标：`llm.operation.duration`、`llm.error.count`、`agent.business.outcome`

**★web合入 P5：LLM 推理 SLO 框架**（来自 A2《使用 OTel 观测 vLLM》）

补充 LLM 推理的 5 类专属指标和 SLO 定义：

| 指标 | 类型 | SLO / 预警 | 说明 |
|---|---|---|---|
| TTFT（首 Token 时间） | Histogram | p95 < 2000ms(流式) / p99 < 3000ms(非流式) | 用户体感最关键指标 |
| ITL（Token 间延迟） | Histogram | p95 < 200ms | 流式输出"卡顿"感知 |
| TPS（每秒 Token 数） | Gauge | — | 吞吐效率 |
| 等待队列深度 `queue_depth` | Gauge | >100 → WARNING | **容量最早预警信号**（比延迟更先触达） |
| 抢占次数 `preemptions` | Counter | growth > 0 → WARNING | **隐性变慢信号**（不报错误但延迟升高） |

> 关键认知（来自 A2）：**队列深度是容量规划的最佳先行指标**——TTFT 升高时往往队列已满，而队列深度是前置信号。**抢占次数是最容易被忽略的"隐性变慢"**——用户体感延迟升高但找不到错误日志时，先查抢占。

**★web合入 P10：AI 自身可观测指标扩展**（来自 N14《AI 接入开发者平台，应先观测后干活》）

我们建设的就是一个银行 AI Agent——观测 AI 自身不只是看业务指标，还要看 AI 运行时的健康指标：

| 指标 | 说明 | SLO 建议 |
|---|---|---|
| **护栏触发次数** | PII/越权/违规内容被 BLOCK 的总次数 | 突增 → CRITICAL |
| **PII 泄露风险** | 日志/Trace 中检测到明文身份证/卡号/手机号的次数 | > 0 → CRITICAL |
| **响应准确率** | 基于 GuideLLM 的 BERTScore(≈0.8) / ROUGE(≈0.4) 作为定量参考 | < 阈值 → WARNING |
| **AI Pod CPU/内存** | AI workload 也是 Pod（非特殊 snowflake） | 标准 K8s 资源告警 |

> PII 护栏实现：LiteLLM 代理层 + Presidio 模式匹配，在 Gateway 层 **BLOCK** 而非 log-alert。在日志/Trace 层面同时做检测告警（双重防线）。

### 4.3a 云原生 7 层黄金信号框架 ★web合入 P17（来自 N10）

> 将现有 RED/USE 指标按 N10 提出的 7 层框架组织，每层有明确的 Owner 和告警策略，避免"指标堆在一层分不清责任"。

| 层 | 典型指标 | Owner | 我们的组件 |
|---|---|---|---|
| L1 资源 | CPU/Memory/Disk/Network | Infra | — |
| L2 编排 | Pod Ready/重启/调度延迟 | SRE | K8s |
| L3 K8s | Deployment/ReplicaSet 状态 | SRE | K8s |
| L4 工作负载 | JVM GC/线程池/连接池 | 后端开发 | Core/Backend |
| L5 平台服务 | PG 连接数/Redis 命中率/Kafka lag | 数据团队 | PG/Redis/Kafka |
| L6 业务服务 | LLM TTFT/准确率/转化率/满意度 | AI 团队 | Tempo+VM |
| **L7 可观测性层自身** | **Collector 丢弃比例 / 接收量基线 / OTLP 导出延迟** | SRE | Collector health |

> **L7 必须被观测**：Collector `dropped_spans` / `refused_spans` / `exporter_send_failed` 指标理想值为 0（除非有意采样丢弃）。

### 4.3b SLO 从业务目标向下拆 · Tier 分级设计 ★web合入 P11（来自 N10/N11）

> **核心原则**：SLO 从业务目标向下拆（用户旅程 → API），不从技术指标向上堆。**禁止给每个内部 API 都配 SLO**，否则回到"一万个自定义告警"的老路。

**银行场景 Tier 分级**：

| Tier | 包含的用户旅程 | SLO 目标 | 错误预算 | 示例 |
|---|---|---|---|---|
| **Tier1** 核心交易 | 登录、转账、支付、账单查询 | 99.9% 可用 / p95 < 3s | 0.1% | 银保监合规必须达标 |
| **Tier2** 重要查询 | 理财推荐、账户详情、风险评估 | 99.5% 可用 / p95 < 5s | 0.5% | 用户体验敏感 |
| **Tier3** 一般交互 | 客服问答、FAQ、满意度反馈 | 99% 可用 / p95 < 8s | 1% | 统计参考 |

**SLI 定义（银行 Tier1 转账为例）**：
- SLI = `successful_transfers / total_transfers`（成功 = HTTP 200 + business outcome=success）
- 测量窗口 = 滚动 7 天
- SLO = 99.9% → 错误预算 = 0.1%（每月约 43 分钟不可用）

**错误预算燃尽告警**：当 Tier1 SLO 的 7 天错误预算消耗超过阈值时触发：
- 5 分钟 burn > 10% → P1（紧急）
- 30 分钟 burn > 5% → P2（警告）
- 24 小时 burn > 1% → P3（通知）

> **内部 API（子图/Agent 网关/RAG 检索）只用异常检测观察，不挂 SLO**——它们的波动不影响业务 SLO，但内部分析需要趋势监控。
...
- **★web合入 P6**：采用**集中式指标注册表**模式（来自 A1）。定义 `ObservabilityMetrics` 结构体集中管理所有 `Meter`/`Counter`/`Timer`/`Gauge`/`UpDownCounter`，命名规范 `deepflux.<domain>.<measure>`（如 `deepflux.agent.intent.accuracy`），提供 `RecordIntentAccuracy(...)` / `RecordReroute(...)` / `RecordBusinessOutcome(...)` 等助手方法。**禁止**在业务代码中直接 `meterRegistry.counter("xxx")` 散落命名。
- **★web合入 P7**：新增 `UpDownCounter` `agent.pending_approval`：人工中断转账审批时 +1、审批完 -1，运营大屏实时显示"当前卡多少条待审批"。
- **★web合入 P8**：引入 OpenLLMetry 补 `gen_ai.*` 语义 span 时，须在 `pom.xml` 钉注版本号并在此文档记录（GenAI 语义存在**版本漂移**：vLLM 用 `prompt_tokens` 非 `input_tokens`，env 属性名 1.27 起改 `deployment.environment.name`，来自 A2）。
...
AI 洞察引擎的结论不能黑盒输出——必须明确"什么能自动做、什么要人确认、什么禁止做"：

| 边界层 | 行为 | 示例 | 实现 |
|---|---|---|---|
| **L1 确定性聚合** | 自动执行（代码） | 统计 P95 时延 / 错误率 / Token 消耗趋势 | InsightsEngine 当前 compute* 方法 |
| **L2 模糊判断** | AI 辅助推荐（需人工确认） | 根因假设生成 / 降噪建议 / 行动优先级排序 | AI 模型推理 → 输出带"置信度"的建议卡 |
| **L3 生产写操作** | **禁止 AI 直接执行** | 回滚部署 / 修改配置 / 扩缩容 / 删除数据 | 必须权限 + 审批 + 审计日志 + 回滚预案 |

> 原则：自动化像扳手（精确可控），AI 像临时同事（给你建议，但你不一定全听）。**无证据链的 AI 结论只是建议，不是指令**（N9）。

**★web合入 P14：MCP 集成**（来自 N9）

AI 洞察引擎仅从 PG/Redis 聚合数据是"离线"的——无法查实时系统状态。应通过 **MCP（模型上下文协议）** 接入真实系统：
- **Prometheus MCP server**：查实时指标（当前 QPS / 错误率 / 延迟分位）
- **Loki MCP server**：查实时日志（某个 trace/session 的最新日志行）
- **ArgoCD MCP server**：查部署历史（"最近一次部署是否与延迟 spike 时间重叠"）
- **PG MCP server**：直接 SQL 查询（替代预定义 API，灵活但需权限管控）

> MCP 像"通用插座"——每个系统装一个 MCP server 转接头，AI 就能跨系统交叉取证。每个结论必须带"证据链"（来自哪个 MCP server、查询了哪些数据、返回了哪些结果）。

**★web合入 P15：RAG / CAG / MCP 三层知识来源**

| 来源 | 内容 | 时效性 | 适用场景 |
|---|---|---|---|
| **RAG**（检索增强生成） | 历史事故报告 / Runbook / 运维文档 | 静态/半静态 | "这个错误过去怎么修的？" |
| **CAG**（上下文增强生成） | 高频场景缓存（如常见 LLM 错误码 → 修复 SOP） | 低频更新 | "LLM 错误率飙升 → 先查限流还是先查模型？" |
| **MCP**（模型上下文协议） | 实时系统状态（指标/日志/部署/配置） | 实时 | "当前 PG 连接数是否接近上限？" |
...
- **版本化**：每个 prompt 模板入库（`insight_reports.prompt_version`），附版本号与变更记录
- **评审**：prompt 变更走 Git PR review 流程，不得直接手动修改
- **测试**：每个 prompt 在已知场景下有预期诊断输出（类似单元测试），CI 中跑 prompt 回归
- **复盘**：洞察结论被证实错误时，追溯该版本 prompt 的缺陷，迭代优化
- **AGENTS.md**：定义每个 AI 职能的指令文件（角色/约束/输出格式/数据源/合规），对应 InsightsEngine 各 Service

> 原则：**提示词即 Runbook**——Runbook 怎么写、prompt 就怎么管（N9）。"版本化的 prompt + 证据链回真实系统"是 AI 洞察不被黑盒化的两条腿。
...
**★web合入 P16：动态基线替代部分静态阈值**（来自 N10/N11）

银行流量昼夜/工作日差异大，纯静态阈值误报多。应以动态基线替代部分静态规则：

| 基线类型 | 适用场景 | 示例 |
|---|---|---|
| **自适应 7 天滚动** | 常态波动 | "Collector 接收量低于 7 天同时间段 P5 值" |
| **多维动态** | 不同端点/区域/客群 | "VIP 转账接口的 TTFT P95 阈值 ≠ 普通用户的阈值" |
| **季节性感知** | 工作日/节假日/促销 | "618 大促期间交易量自动放宽 3 倍" |
| **静态阈值（保留）** | 硬性上限 | "PG 连接数 > 200 → CRITICAL"（无动态调整余地） |

> 建议：**70% 用动态基线、30% 用静态硬上限**。Prometheus `predict_linear()` 或 VictoriaMetrics `rollup_rate()` 可实现基础动态基线。

**★web合入 P18：SLO 影响分析降噪**（来自 N10/N6 — 降噪可达 90%）

银行生产环境告警量庞大，最有效的降噪判据是"**是否消耗关键业务 SLO 错误预算**"：
- **SLO 影响分析**：告警触发时，自动评估该告警所关联的服务是否正在消耗 Tier1 错误预算。只在"消耗错误预算"时才升 P0，否则降 P3 或仅通知。
- **告警聚合 4 维**：横向调用链（同一用户旅程的上下游告警合并）/ 纵向运行（同一服务的历史重复告警合并）/ 网络连接（同一集群的关联告警）/ 跨应用依赖（DB→App→用户的前向影响）
- **Burn rate 告警**（来自 N6）：5min burn > 10% → P1 / 30min burn > 5% → P2 / 24h burn > 1% → P3
- **退役面向客户日志告警**（来自 N11）：把"查日志发现异常"改为"查指标发现异常→再查日志"，消除日志扫描告警
...
**★web合入 P22：银行采样细化**（来自 N10）

银行交易中成功路径占 99.9%，全部保留细节造成巨大存储浪费。采用分级策略：

| 场景 | 采样策略 | 保留内容 |
|---|---|---|
| **交易成功 + 正常耗时** | **头部采样 10%** | 完整 trace + 日志，其余仅保留聚合指标 |
| **交易成功 + 慢（>P95）** | **100%** | 完整 trace + 日志，供根因分析 |
| **交易成功 + 监管相关 intent** | **100%** | 转账/支付等合规场景，完整保留 |
| **任何错误/超时** | **100%（尾部采样）** | 完整 trace + 日志，故障复盘 |
| **指标** | **100%**（非采样） | 所有指标无采 — 只采样 trace/logs |

### 11.3 生产增强配置 ★web合入 P1/P2/P9

**11.3.1 P1：Kafka 缓冲层**

银行量级下 Collector 直连存储有三大风险：存储被冲垮、Collector 重启丢数据、无法回放。须在 Collector 和存储之间插入 Kafka 作为削峰/解耦/回放缓冲：
...
exporters:
  kafka:
    ...
    topic: otel-spans
    encoding: otlp_json
    # 生产者确保消息持久化后才确认（ACK=all）
    producer:
      acks: all
      compression: snappy
```

数据流：`App → Collector → Kafka → (Tempo Consumers / VM Consumers / Loki Consumers / PG Consumer)`。Kafka 保留 24h，支持按时间回放。

**11.3.2 P2：batch processor 生产纠偏**

V6 原用 `batch` processor 在生产环境有**静默丢数据风险**——它在数据持久化前就确认接收，Collector 重启时仍在内存中的批次会丢失。社区已建议弃用 batch、改用持久化 exporter 级批处理：

```yaml
# 替代方案：file_storage extension + persistent queue
extensions:
  file_storage:
...
exporters:
  kafka:
    # ... 如上 Kafka 配置
    sending_queue:
      storage: file_storage        # 磁盘持久化队列，重启不丢
      queue_size: 10000
      num_consumers: 10
```

> **P0 Demo 阶段可保留 batch**（单机不重启），但本文档已标记此为生产必改项。

**11.3.3 P9：Collector DaemonSet 部署**

生产 K8s 环境下，除 Gateway 集群（3 实例 + LB）外，每节点还应部署 DaemonSet 采集节点级数据（kubelet/cAdvisor 指标、节点日志），Sidecar 模式留给高隔离租户。

| 模式 | 用途 | 场景 |
|---|---|---|
| **Gateway 集群** | 应用遥测入口，LB 分发 | 所有 Core 实例的 OTLP 目标 |
| **DaemonSet** | 节点级指标/日志采集 | kubelet / cAdvisor / 容器日志 |
| **Sidecar** | 高隔离租户 | 合规要求严格隔离的特定应用 |
...
### 13.5 PII 网关级 BLOCK 防线 ★web合入 P10（来自 N14）

> V6 原 §13.1 脱敏在 Core 端完成（发送前就打码），这是**第一道防线**。但文本经过 LLM/Agent 多层处理后可能还原，需要**第二道防线**：在 Logs / Trace 出口处做网关级 BLOCK。

| 防线 | 位置 | 机制 | 动作 |
|---|---|---|---|
| **L1 Core 端脱敏**（已有） | Core SessionBridge / ObsChatModel | 正则脱敏卡号/身份证/手机号 | 发送前打码 |
| **L2 网关级 BLOCK** ★新增 | LiteLLM 代理层 + Presidio | PII 模式匹配 + NER 识别 | **BLOCK 请求**（不发给 LLM）+ 记录护栏触发事件 |
| **L3 Trace/Log 检测** ★新增 | Collector `redaction` processor 或日志采集端 | 扫描 span attributes / log body | **自动脱敏或拒绝写入**，触发 PII 泄露告警 |

```
用户查询（含卡号）
  → L1 Core: 脱敏打码 6228****1234
  → L2 LiteLLM+Presidio: 二次检测 → 若仍被 LLM 还原或泄漏 → BLOCK
  → L3 OTel Collector: 最终防线 → 扫描写入 Tempo/Loki 的数据 → 拒绝/脱敏
```

> 实现：LiteLLM 代理 + Presidio（Microsoft 开源 PII 检测），识别身份证/银行卡/手机号/SSN/医疗许可编号，在请求发给 LLM 前拦截。护栏触发次数、PII 泄露次数均作为指标上报（见 §4.3 ★P10）。**双重防线：脱敏是"先打码再发给 LLM"，护栏是"检测到就不发"**。
...
| Collector | 4C 8G | 3（Gateway）+ DaemonSet（每节点） |
| **Kafka** ★web P1 | 8C 32G + 2TB SSD | 3（replication=3, min.insync=2） |
| VM 存储节点 | 8C 32G + 1TB SSD | 3 |
| 对象存储（MinIO/S3） | — | 按 3y 冷归档估算 |
| 后端 | 4C 8G | 2 |

### 14.3 银行金融落地路线图 ★web合入 P12（来自 N11「时序折叠」）

> N11 虚构了一家金融服务公司"时序折叠"的完整案例：云迁移后组件数 ×80 → 告警暴涨 / 可追踪性丢失 / 用户先发现事故 / 缺 Owner。这几乎就是我们的镜像。其 **crawl → walk → run** 三阶段可 1:1 复用作我们的生产部署路线图：

| 阶段 | 目标 | 关键动作 | 对应我们 P0-P2 |
|---|---|---|---|
| **Crawl** 先站稳 | ① 补 Owner 元数据 ② 统一 OTel 埋点 ③ 建 5 条信号管道 | 强制 CI 检查 Owner label；K8s label/annotation 标准化；所有服务接 OTel Agent | P0 Core 埋点 + PG 迁移 |
| **Walk** 能看准 | ① 告警从静态 → 动态基线 ② 退役"面向客户日志告警" ③ SLO 从业务向下拆 | 迁 Grafana Alerting；指标化告警；Tier1/2/3 分级 | P1 Collector + 标准栈 + SLO 分级 |
| **Run** 可预防 | ① AI 辅助（异常检测 → 根因推荐）② AI 执行（回滚/切换/扩容，人工确认）③ 成本治理（采样/分层保留） | MCP 集成 + InsightsEngine；安全护栏 L3 级管控；冷热分层 | P2 告警迁 Grafana + P3 扩展 |

> **"时序折叠"的核心教训与我们的对应**：
> - ❌ 教训：告警暴涨 + 用户先发现事故 → ✅ 我们：SLO 从业务向下拆 + P18 降噪 90%
> - ❌ 教训：面向客户的日志告警延迟 15min → ✅ 我们：指标化告警 + Kafka 24h 缓冲
> - ❌ 教训：缺 Owner → ✅ 我们：P19 K8s label 强制 Owner + ODD
> - ❌ 教训：自建 vs 购买 ✗ → ✅ 我们：全开源 Grafana 栈 + Alloy （已验证 15 篇文章无反驳）
...
### 15.5 平台治理 ★web合入 P19（来自 N10/N12）

> 可观测性不是搭好平台就完——**元数据是"语法"，Owner 是"主语"，ODD 是"流程"**。

**15.5.1 Owner 元数据强制**

所有 K8s 资源必须带以下 label/annotation：

```yaml
labels:
  Environment: production
  Application: bank-agent
  Version: v1.2.3
  OwningTeam: ai-platform
annotations:
  CostCenter: CC-0001
  Priority: high
  AlertingMode: pagerduty
```

强制手段：**CI/CD 检查（无 Owner label 不允许构建）+ Kyverno 准入控制器（无 Owner label 不允许创建）**。

**15.5.2 可观测性驱动开发（ODD）**

将可观测性当作软件需求的一部分（类似 TDD）：

1. 写 SLO 目标（如"转账 API p95 < 3s"）
2. 定义 SLI（成功转账数 / 总转账数）
3. 部署前确认 SLI 可采集（OTel 埋点就绪）
4. 上线后验证 SLO 达标，不达标 → 阻塞后续发布

> 指标采集间隔：critical（交易）10s / normal（查询）30s / low（统计）60s（来自 N7）。

### 15.6 多团队推广（可选） ★web合入 P21（来自 N12/N13/N15）

> 若可观测平台需推广给银行内部多个开发团队，需走"产品化"路线而非"传帮带"。

| 推广要素 | 做法 | 来源 |
|---|---|---|
| **IDP（内部开发者平台）** | Backstage 等门户，创建服务时自动生成日志结构/指标暴露/OTel 示例/PrometheusRule/GrafanaDashboard/K8s label/Owner 元数据/SLO | N12 |
| **Helm 标准 chart** | 统一 chart 包含 ServiceMonitor + PrometheusRule + Grafana Dashboard + OTel 路由 | N13 |
| **模板生命周期** | Git 管理 + Renovate Bot 自动提 PR 更新模板版本 | N13 |
| **Kyverno 策略** | 强制 Owner label / OTel sidecar / 日志格式 | N13 |
| **成熟度模型** | 5 级：L0 无观测 → L1 基础指标 → L2 全链路 Trace → L3 SLO + 告警 → L4 AI 洞察 | N15 |
| **自助边界** | 开发者自助查：关键 Dashboard 链接 / 最近关键日志 / SLO 概览 / 性能热点自动分析 / 可观测性记分卡；但**不能**删数据/改配置 | N15 |

> 这些属于"多团队产品化"范畴，应在 Demo 验证通过、银行内部确定推广后启动。不抢占 P0-P2。
...
> 文档版本：0711 · WorkBuddy V6 · web文章合入（整合终稿 · 银行业务量级 · 15 篇外部文章全量注入）
> 融合路径：Wb V2 + Codex V2 → Wb V3 + Codex V3 → Wb V4 → Wb V5（银行量级修正）→ Wb V6（整合 Codex V4 银行创新）→ **web文章合入**（15 篇微信文章，P1–P22 全量）
> V6 新增：数据脱敏 / 分级采样 / TDigest / WebSocket / 物化视图 / TLS / RBAC / 审计 / 合规章 / HA 章 / 银行告警规则
> web合入新增：Kafka 缓冲层 / batch 纠偏 / Exemplar / HITL 会话连续性 / LLM SLO 框架 / 集中指标注册表 / UpDownCounter / GenAI 版本钉注 / DaemonSet / AI 自身可观测+PII 网关级 BLOCK / SLO Tier 分级 / 时序折叠蓝图 / 三层边界 / MCP+RAG+CAG / 动态基线 / 7 层黄金信号 / SLO 降噪 / ODD+Owner / 提示词即 Runbook / IDP 推广 / 采样细化
> 外部文章：`docs/外部参考文章与自有方案对比分析.md`（15 篇，§1–§8，含 §8.5 Demo/生产分界表）
