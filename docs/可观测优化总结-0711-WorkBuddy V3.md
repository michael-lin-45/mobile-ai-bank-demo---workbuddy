# 可观测优化总结（0711 · WorkBuddy V3 · 合并终稿）

> **合并原则**：取 WorkBuddy V2 的全景架构 + 组件选型 + UI 布局之长，补 Codex V2 的代码级实现 + 引擎架构 + 维度规范之长。
> 适用范围：移动 AI 银行 Demo（Core 8080 → OTel Collector 4318 → Backend 9090 / H2+Redis → React 前端 + 自研大屏 v20）
> 合并依据：`docs/可观测设计方案差异分析-WorkBuddy.md`（14 模块逐项比对）
...
| D7 | AI 洞察用 InsightsEngine 统一引擎而非分散 Service | 交叉分析+根因定位+行动建议，避免每个 TAB 各自算 | Codex |
...
## 9. AI 洞察引擎（InsightsEngineService，Codex §9 融入 V3）
...
> 文档版本：0711 · WorkBuddy V3（合并终稿）
> 合并来源：WorkBuddy V2 (`docs/可观测优化总结-0711-WorkBuddy V2.md`) + Codex V2 (`docs/specs/可观测优化总结-0711-Codex-V2.md`)
> 差异分析：见 `docs/可观测设计方案差异分析-WorkBuddy.md`
