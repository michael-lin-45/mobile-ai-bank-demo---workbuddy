 ...
 *
 * 设计(自管状态版):
 * - 自管 billActiveThreads: 本领域独立的activeThread，不与其它领域共享
 * - 无suspendedAgents: 1-1关系，只有一个子智能体，无需挂起/恢复
 * - FOLLOW_UP + 自有activeThread → resumeGraph(注入accumulatedParams)
 ...
 *
 * 关键bug修复(与TransferService一致):
 * 之前用全局activeThread，用户从账单切到转账再切回时，账单的accumulatedParams丢失。
 * 现在账单自管activeThread，切走再回来时上下文还在。
 */
/**
 * 账单L1 Service - @Deprecated: 已迁移到SingleSubAgentDomainService (通过DomainServiceConfig配置)
 *
 * 保留此文件仅供参考，不再由Spring容器管理。
 * 新的实现: DomainServiceConfig.billDomainService()
 *
...
public class BillService {
    ...
    /** 账单领域自有的activeThread (per session) */
    private final Map<String, ActiveThreadInfo> billActiveThreads = new ConcurrentHashMap<>();
    ...
    public static class ActiveThreadInfo {
        ...
        /** 累积的参数 (如bill.billType, bill.timeRange等) */
    ...
    private ActiveThreadInfo getOwnActiveThread(String sessionId) {
        return billActiveThreads.get(sessionId);
    }

    private void setOwnActiveThread(String sessionId, String threadId, String intent) {
        if (threadId == null || intent == null) {
            billActiveThreads.remove(sessionId);
        } else {
            billActiveThreads.put(sessionId, new ActiveThreadInfo(threadId, intent, Instant.now()));
        }
    }
     ...
     * 2. FOLLOW_UP + 自有activeThread → resumeGraph(注入accumulatedParams)
    ...
    public WorkflowOutput handle(String sessionId, String userInput) {
        ...
        try {
            ...
            if (ownActive != null) {
                log.info("[BillService] FOLLOW_UP (activeThread exists): intent={}, params={}",
            ...
            log.info("[BillService] Phase1 (no activeThread): routeType={}, confidence={}", phase1.getRouteType(), phase1.getConfidence());
    ...
    private void saveAccumulatedParams(String sessionId, WorkflowOutput output) {
        ...
        if (active != null && !output.getAccumulatedParams().isEmpty()) {
            ...
            log.info("[BillService] Saved accumulated params: {}", output.getAccumulatedParams());
        }
    }
     ...
     * 取消当前账单操作
     */
    public WorkflowOutput cancelBill(String sessionId) {
        ActiveThreadInfo active = getOwnActiveThread(sessionId);
        if (active == null) {
