...
public class BillService {
    ...
    public WorkflowOutput handle(String sessionId, String userInput) {
        ...
        try {
            ...
            if (phase1.isFollowUp()) {
                ...
                log.info("[BillService] FOLLOW_UP but no activeThread → fallback to SWITCH_NEW");
            }
    ...
    private void recordSystemReply(String sessionId, WorkflowOutput output) {
        ...
        if (reply != null && !reply.isEmpty()) {
            billChatMemory.add(sessionId, new AssistantMessage(reply));
            log.debug("[BillService] Recorded system reply: session={}, reply={}", sessionId, truncate(reply, 50));
        }
    }
