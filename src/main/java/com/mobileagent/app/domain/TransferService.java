        ...
        try {
            ...
            if (phase1.isFollowUp()) {
                ...
                if (active != null && INTENT.equals(active.getIntent())) {
                    log.info("[TransferService] FOLLOW_UP with same-domain activeThread: intent={}", active.getIntent());
                    transferChatMemory.add(sessionId, new UserMessage(userInput));
                ...
                if (active != null) {
                    log.info("[TransferService] FOLLOW_UP but activeThread is different domain: activeIntent={}, thisDomain={} → suspend and SWITCH_NEW",
