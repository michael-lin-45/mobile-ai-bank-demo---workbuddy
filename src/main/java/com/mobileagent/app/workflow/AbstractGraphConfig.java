...
public abstract class AbstractGraphConfig {
    ...
    protected boolean detectCancelFromInput(OverAllState state) {
        ...
        String prompt = String.format("""
            你是手机银行智能助手。判断用户是否想取消/放弃当前操作。

            当前场景: %s
            用户输入: %s

            判断标准:
            - 取消: 用户明确表达不想继续当前操作(如"取消""算了""不要了""不查了""不转了""别转了""放弃")
            - 取消: 用户表达的否定意图针对当前操作, 而非回答问题
            - 不是取消: 用户只是暂时切换到其他任务(如"先看看账单""先查一下""先转个账")，"先X"表示用户要先做另一件事，不是取消当前操作
            - 不是取消: 简单的否定回答(如"不是""不对")
            - 不是取消: 用户提出了其他领域的请求(如"查账单""推荐理财")，这是意图切换不是取消
